 ...
 * <a href="https://app.codility.com/programmers/lessons/14-binary_search_algorithm/min_max_division/">...</a>
 */
public class MinMaxDivision {
    public int solution(int K, int M, int[] A) {
        ...
        int maxSum = 0;
        for (int j : A) {
            maxSum = maxSum + j;          // maxSum: sum of all elements
            minSum = Math.max(minSum, j); // minSum: at least one max element
        }
    ...
    public boolean checkDivisable(int mid, int k, int[] a) {
        ...
        for (int j : a) {
            currentBlockSum = currentBlockSum + j;
            if (currentBlockSum > mid) { // means: need one more block
                numBlockAllowed--;
                currentBlockSum = j; // note: next block
            }
